...
func (p *Plugin) createGmeetCommand() (*model.Command, error) {
	iconData, err := command.GetIconData(p.API, "assets/icon.svg")
	if err != nil {
		return nil, errors.Wrap(err, "failed to get icon data")
	}
	return &model.Command{
		Trigger:              gmeetCommand,
		AutoComplete:         true,
		AutoCompleteDesc:     "Start a Google meeting in current channel. Other available commands: start, help, settings",
...
func getAutocompleteData() *model.AutocompleteData {
	gmeet := model.NewAutocompleteData(gmeetCommand, "[command]", "Start a Google meeting in the current channel. Other available commands: start, help")

	start := model.NewAutocompleteData(gmeetStartCommand, "[topic]", "Start a new meeting in the current channel")
	start.AddTextArgument("(optional) The topic of the new meeting", "[topic]", "")
	gmeet.AddCommand(start)

	help := model.NewAutocompleteData("help", "", "Get slash command help")
	gmeet.AddCommand(help)

	settings := model.NewAutocompleteData("settings", "[setting] [value]", "Update your user settings (see /gmeet help for available options)")

	see := model.NewAutocompleteData(gmeetSettingsSeeCommand, "", "See your current settings")
	...
	namingScheme.AddStaticListArgument("Choose the naming schema of the Google Meet link", true, items)
	settings.AddCommand(namingScheme)
	gmeet.AddCommand(settings)
...
func (p *Plugin) executeStartMeetingCommand(_ *plugin.Context, args *model.CommandArgs) (*model.CommandResponse, *model.AppError) {
	input := strings.TrimSpace(strings.TrimPrefix(args.Command, "/"+gmeetCommand))
	input = strings.TrimSpace(strings.TrimPrefix(input, gmeetStartCommand))
...
func (p *Plugin) executeHelpCommand(_ *plugin.Context, args *model.CommandArgs) (*model.CommandResponse, *model.AppError) {
	...
	helpTitle := p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
		DefaultMessage: &i18n.Message{
			ID: "gmeet.command.help.title",
			Other: `###### Mattermost Google Meet Plugin - Slash Command help
`,
		},
	})
	commandHelp := p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
		DefaultMessage: &i18n.Message{
			ID: "gmeet.command.help.text",
			Other: `* |/gmeet| - Create a new meeting
* |/gmeet start [topic]| - Create a new meeting with specified topic
* |/gmeet help| - Show this help text
* |/gmeet settings see| - View your current user settings for the Google Meet plugin
* |/gmeet settings [setting] [value]| - Update your user settings (see below for options)
...
func (p *Plugin) executeSettingsCommand(_ *plugin.Context, args *model.CommandArgs, parameters []string) (*model.CommandResponse, *model.AppError) {
	...
	if err != nil {
		...
		return p.settingsError(args.UserId, args.ChannelId, p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "gmeet.command.settings.unable_to_get",
				Other: "Unable to get user settings",
			},
		}), args.RootId)
	}

	if len(parameters) == 0 || parameters[0] == gmeetSettingsSeeCommand {
		text = p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID: "gmeet.command.settings.current_values",
				Other: `###### Google Meet Settings:
* Naming Scheme: |{{.NamingScheme}}|`,
	...
	if len(parameters) != 2 {
		return p.settingsError(args.UserId, args.ChannelId, p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "gmeet.command.settings.invalid_parameters",
	...
	case commandArgNamingScheme:
		...
		default:
			text = p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
				DefaultMessage: &i18n.Message{
					ID:    "gmeet.command.settings.wrong_naming_scheme_value",
	...
	default:
		text = p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "gmeet.command.settings.wrong_field",
				Other: "Invalid config field, use `naming_scheme`.",
			},
		})
		userConfig = nil
	}
	...
	if err != nil {
		...
		return p.settingsError(args.UserId, args.ChannelId, p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "gmeet.command.settings.unable_to_set",
	...
	post := &model.Post{
		...
		Message: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "gmeet.command.settings.updated",
				Other: fmt.Sprintf("Gmeet settings updated:\n\n* %s: `%s`", parameters[0], parameters[1]),
			},
		}),
		RootId: args.RootId,
	}
